#!/usr/bin/env python3
from builtins import format, print
import os
import cv2
import random
import torch
...
from utils_my.ip_basic.ip_basic import vis_utils
from utils_my.ip_basic.ip_basic import depth_map_utils_ycb as depth_map_utils
...
intrinsic_matrix = {
    ...
    'ycb_K2': np.array([[1077.836, 0.        , 323.7872],
                        [0.      , 1078.189  , 279.6921],
                        [0.      , 0.        , 1.0]], np.float32),
    'MP6D_K': np.array([[567.5372, 0.        , 312.6657],
                        [0.      , 569.3618  , 257.1730],
                        [0.      , 0.        , 1.0]], np.float32)
}
...
class Basic_Utils():
    ...
    def __init__(self, config):
        ...
        if config.dataset_name == "ycb" or config.dataset_name == "MP6D":
    ...
    def project_p3d(self, p3d, cam_scale, K=intrinsic_matrix['MP6D_K']):
        if type(K) == str:
    ...
    def get_cls_name(self, cls, ds_type):
        if type(cls) is int:
            #if ds_type == 'ycb':
            if ds_type == 'ycb' or ds_type == 'MP6D':
    ...
    def get_pointxyz_full(
        ...
        if ds_type == "ycb":
            ...
            ptxyz_ptn = os.path.join(
                self.config.ycb_root, 'models',
                '{}/points_full_uniform_8192.xyz'.format(cls),
            )
            pointxyz = np.loadtxt(ptxyz_ptn.format(cls), dtype=np.float32)
            # print("BSU ycb path in get_pointxyz_full eval: {}".format(ptxyz_ptn))
            ...
            if cls in self.ycb_cls_ptsxyz_dict.keys():
                return self.ycb_cls_ptsxyz_dict[cls]
            ptxyz_ptn = os.path.join(
                self.config.ycb_root, 'models_cad',
                '{}_full_uniform_8192.xyz'.format(cls),
            )
            pointxyz = np.loadtxt(ptxyz_ptn.format(cls), dtype=np.float32)
            # print("BSU ycb path in get_pointxyz_full eval: {}".format(ptxyz_ptn))
            self.ycb_cls_ptsxyz_dict[cls] = pointxyz
    ...
    def get_pointxyz(
        ...
        if ds_type == "ycb":
            ...
            ptxyz_ptn = os.path.join(
                self.config.ycb_root, 'models',
                '{}/points_full_uniform_1024.xyz'.format(cls),
            )
            pointxyz = np.loadtxt(ptxyz_ptn.format(cls), dtype=np.float32)
            #print("pointxyz size ycb: {0}".format(len(pointxyz)))
            self.ycb_cls_ptsxyz_dict[cls] = pointxyz
            return pointxyz
        ...
        ################## ADD BY ZJ FOR MP6D ####################
            ...
            # print("point xyz path: {}".format(ptxyz_ptn))
            pointxyz = np.loadtxt(ptxyz_ptn.format(cls), dtype=np.float32) # 归一化单位 （m）
            dellist = [j for j in range(0, len(pointxyz))]
            if len(pointxyz) > 20000:
                num_sample = 20000
            else:
                num_sample = len(pointxyz)
            # dellist = random.sample(dellist, len(pointxyz) - num_sample)
            # pointxyz = np.delete(pointxyz, dellist, axis=0)
            # print("pointxyz size: {0}".format(len(pointxyz)))
            self.ycb_cls_ptsxyz_dict[cls] = pointxyz
            return pointxyz
        ...
        ################## ADD BY ZJ FOR MP6D ####################
            ...
            dellist = [j for j in range(0, len(pointxyz))]
            dellist = random.sample(dellist, len(pointxyz) - 512)
            pointxyz = np.delete(pointxyz, dellist, axis=0)
    ...
    def get_kps(
        ...
        ##################### add by zj for mp6d ########################
            ...
            # ) # 如果是MP6D数据集，则使用八个角点作为关键点
            # print("BSU MP6D kps_pth in get_kps eval: {}".format(kps_pth))
            if use_orbfps:
                kps_pth = self.config.kp_orbfps_ptn % (cls, self.config.n_keypoints)
            else:
                kps_pth = os.path.join(
        ...
        ##################### add by zj for mp6d ########################    
            ...
            #print("BSU LINEMOD kps_pth in get_kps:", kps_pth)
            kps = np.loadtxt(kps_pth, dtype=np.float32)
    ...
    def get_ctr(self, cls, ds_type='ycb', ctr_pth=None):
        ...
        if ds_type == 'ycb' or ds_type == 'MP6D':
            ...
            # print("BSU MP6D _corners in _corners:", cor_pattern)
            cors = np.loadtxt(cor_pattern.format(cls), dtype=np.float32)
            ctr = cors.mean(0)
            
            # print("BSU MP6D cor_pattern in cor_pattern eval: {}".format(cor_pattern))
