...
def of_l1_loss(
        pred_ofsts, kp_targ_ofst, labels, pred_ofsts_score,
    ...
    abs_diff = abs_diff * pred_ofsts_score - 0.01 * torch.log(pred_ofsts_score)
    
    abs_diff = w * abs_diff
    # if True in torch.isnan(abs_diff) or True in torch.isinf(abs_diff):
    #     print("abs_diff is: {0}".format(abs_diff))
    #     print("pred_ofsts_score is: {0}".format(pred_ofsts_score))

    abs_diff[torch.isinf(abs_diff)] = 1.0
    # abs_diff[torch.isnan(abs_diff)] = 0.0
    in_loss = abs_diff
...
class OFLoss(_Loss):
    ...
    def forward(
        self, pred_ofsts, kp_targ_ofst, labels, pred_ofsts_score,
...
class CosLoss(_Loss):
    ...
    def forward(
        ...
        cos_sim = pred_vec * targ_vec #[-1,1]
        
        cos_sim = 1.0 - cos_sim # add by zj [0,2]
        in_loss = w * cos_sim
        # in_loss = -1.0 * w * cos_sim
        # print("w: ", w)
        # print("in_loss: ", in_loss)
